...
Deno.serve(async (req) => {
  ...
  if (body.event_type) query = query.ilike("event_type", `%${body.event_type}%`);
  ...
  return new Response(JSON.stringify({ ok: true, me: { user_id: auth.userId, role: auth.role }, events: data }), {
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  });
});
